...
// Torn-Fehlercode 17 ("Backend error occurred, please try again") ist ein
// bekanntes, temporäres Problem auf Torns Seite. Torn empfiehlt selbst,
// die Anfrage einfach erneut zu senden.
const RETRYABLE_TORN_ERROR_CODES = new Set([17]);
const MAX_RETRIES = 2;
...
// Zentraler Torn-API-Aufruf mit automatischem Retry bei bekannten,
// vorübergehenden Backend-Fehlern (z.B. Code 17).
async function fetchTornApi(url) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
        ...
        const isRetryable = RETRYABLE_TORN_ERROR_CODES.has(data.error.code);
        ...
        logger.warn(`[TornLogTracker] Torn-API-Fehler ${data.error.code} (Versuch ${attempt + 1}/${MAX_RETRIES + 1}), wiederhole in ${RETRY_DELAY_MS}ms...`);
...
export async function verifyTornApiKey(apiKey) {
    ...
    const data = await fetchTornApi(url);
...
function buildEmbed(entry, discordUserId) {
    const embed = new EmbedBuilder()
        .setTitle(entry.title || 'Torn Log Entry')
        .setColor(0x5865f2)
        .setFooter({ text: `Member: ${discordUserId}` });
...
function buildOnlineEventEmbed(discordUserId, type) {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`<@${discordUserId}> is now **${isOnline ? 'online' : 'offline'}**.`)
        .setColor(isOnline ? 0x43b581 : 0x747f8d);
}

function buildDailySummaryEmbed(discordUserId, dateStr, totalSeconds) {
    return new EmbedBuilder()
        .setTitle('📊 Daily Summary: Online Time')
        .setDescription(`<@${discordUserId}> was online for a total of **${formatDuration(totalSeconds * 1000)}** on **${dateStr}**.`)
        .setColor(0x5865f2);
}
